...
public class ClientStream extends SimulationObject {
    ...
    List<Client> klienciDoUsuniecia = new ArrayList<>();
    List<Client> klientID = new ArrayList<>();
    Simulation simulation;
    int timeToEqual;

    Uniform uniformObject = new Uniform(4, 7);
    Uniform prawdZdenerwowania;
    RoundRobin roundRobinObject = new RoundRobin(new int[]{8, 7, 5}); // NORMALNY PRZYPADEK
//    RoundRobin roundRobinObject = new RoundRobin(new int[]{1, 1, 15}); //  PRZYPADEK GDY KAZA ZOSTANIE NA POCZATKU ZAMKNIETA ZE WZGLEDU NA BRAK KLIENTOW
    ...
    public void newClient(int next) {
        int i = 0;
        while (i < next) {
